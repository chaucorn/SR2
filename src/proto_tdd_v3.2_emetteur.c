/*************************************************************
* proto_tdd_v3.2 -  émetteur                                   *
* TRANSFERT DE DONNEES  v3                                   *
*                                                            *
* Protocole Go-Back-N FAST TRANSMITARQ  *
*                                                            *
* E. Lavinal - Univ. de Toulouse III - Paul Sabatier         *
**************************************************************/


#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include "application.h"
#include "couche_transport.h"
#include "services_reseau.h"
/* =============================== */
/* Programme principal - émetteur  */
/* =============================== */


int main(int argc, char* argv[]){
    int window_size = 4; // Prends la valeur 4 par défaut
    if (argc > 2) {
        printf("Usage: %s <window>\n", argv[0]);
        return 1;
    }
    if (argc == 2) { // Si un argument est passé, on le prend comme taille de la fenêtre
        window_size = atoi(argv[1]);
        if (window_size < 1 || window_size > (SEQ_NUM_SIZE-1)/2) {
        printf("Erreur: Taille Fenetre invalide\n");
        return 1;
    }
    }

    unsigned char message[MAX_INFO]; /* message de l'application */
    int taille_msg; /* taille du message */
    paquet_t tab_p[SEQ_NUM_SIZE]; 
    paquet_t pack;
    int borne_inf = 0;
    int curseur = 0;
    int evt;
    int last_ack = -1;      // last ACK number received
    int dup_count = 0;       // how many times we received this ACK
    
    //int evt = 0; // 0: timeout  1: paquet arrivé
    init_reseau(EMISSION);

    printf("[TRP] Initialisation reseau : OK.\n");
    printf("[TRP] Debut execution protocole transport.\n");

    /* lecture de donnees provenant de la couche application */
    de_application(message, &taille_msg);

    /* tant que l'émetteur a des données à envoyer */
    while (taille_msg != 0 || borne_inf != curseur) {

        // If we can send a new packet in the window
        if (dans_fenetre(borne_inf, curseur, window_size) && taille_msg > 0) {
            // Fill the packet
            paquet_t *p = &tab_p[curseur];
            for (int i = 0; i < taille_msg; i++) 
                p->info[i] = message[i];

            p->num_seq = curseur;
            p->lg_info = taille_msg;
            p->type = DATA;
            p->somme_ctrl = generer_controle(p);

            printf("[DEBUG] Sending packet seq=%d, taille=%d\n",
                p->num_seq, p->lg_info);
            vers_reseau(p);

            if (borne_inf == curseur)
                depart_temporisateur(100);

            curseur = inc(curseur, SEQ_NUM_SIZE);

            // Read next message
            de_application(message, &taille_msg);
        }
        else {
            // Wait for ACK or timeout
            evt = attendre();

            if (evt == PAQUET_RECU) {
                de_reseau(&pack);
                if (verifier_controle(&pack) && dans_fenetre(borne_inf, pack.num_seq, window_size)) {

                    // Slide window
                    borne_inf = inc(pack.num_seq, SEQ_NUM_SIZE);

                    // Stop timer if all packets acknowledged
                    if (borne_inf == curseur)
                        arret_temporisateur();
                    // FAST RETRANSMIT LOGIC
                    if (pack.num_seq == last_ack) {
                        dup_count++;
                    } else {
                        last_ack = pack.num_seq;
                        dup_count = 1;   // first time we see this ACK
                    }

                    if (dup_count == 3) {
                        printf("[DEBUG] Fast Retransmit triggered for seq=%d\n", last_ack + 1);
                        int i = inc(last_ack, SEQ_NUM_SIZE);
                        while (i != curseur) {
                            vers_reseau(&tab_p[i]);
                            i = inc(i, SEQ_NUM_SIZE);
                        }
                        depart_temporisateur(100);
                    }
                        
                }
            }
            else { // Timeout: retransmit only initialized packets
                int i = borne_inf;
                while (i != curseur) {
                    if (tab_p[i].lg_info > 0) { // <--- SAFE CHECK
                        printf("[DEBUG] Retransmitting seq=%d, taille=%d\n",
                            tab_p[i].num_seq, tab_p[i].lg_info);
                        vers_reseau(&tab_p[i]);
                    }
                    i = inc(i, SEQ_NUM_SIZE);
                }
                depart_temporisateur(100);
            }
        }
    }
    printf("[TRP] Fin execution protocole transfert de donnees (TDD).\n");
    return 0;
    
    
}


