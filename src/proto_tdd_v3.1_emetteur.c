        ...
        if (dans_fenetre(borne_inf, curseur, window_size) && taille_msg > 0) {
            ...
            if (evt == PAQUET_RECU) {
                ...
                if (verifier_controle(&pack) &&
                    dans_fenetre(borne_inf, pack.num_seq, window_size)) {
...
//             printf("[DEBUG] Retransmitting window from %d to %d\n", borne_inf, curseur);
...
//                     int i = borne_inf;
//                     while (i != curseur) {
//                         if (tab_p[i].lg_info > 0) {
//                             printf("[DEBUG] Retransmitting seq=%d\n", tab_p[i].num_seq);
//                             vers_reseau(&tab_p[i]);
//                         }
//                         i = inc(i, SEQ_NUM_SIZE);
//                     }
...
//             }// <-- closes while(borne_inf != curseur)
